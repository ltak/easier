...
2. App shows today summary, strongest issue, and best next change.
3. User taps `Log Meal` or opens an existing meal.
4. Meal Editor updates from mocked analysis responses on edit.
...
2. App shows next workout, recent completion, and recovery context.
3. User opens a workout session.
4. User logs sets and completes the session.
5. Stub service returns progression decision and updated summary state.

#### Health tab flow

1. User lands on Health Home.
2. App shows weight trend, sleep/activity summary, and readiness context.
3. User drills into trends or connected sources.
...
5. Health Home + trend slice
6. Goal/projection cards embedded into Health or Nutrition
7. Coaching cards embedded across tabs
